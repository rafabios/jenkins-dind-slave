...
RUN export GIT_SSL_NO_VERIFY=1 && \
    git clone https://spobvokd1001.indusval.com.br/root/templates-utils.git && \
     cd templates-utils && \
     /bin/bash setup.sh

# Install python tools
RUN apt-get update && apt-get install -y python-pip git python python-yaml && pip install python-consul
